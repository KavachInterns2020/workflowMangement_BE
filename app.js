...
app.get("/getWorkFlow", (req, res1) => {
    ...
    con.query('SELECT workflow_workflow_id from workflow.users as u,workflow.users_has_workflow as ur,workflow.workflow as w where u.user_id = ur.Users_user_id and ur.workflow_workflow_id = w.workflow_id and u.username = ?', [user], (req, res) => {
        async.each(res, (res, callback) => {
            con.query('select workflow_name from workflow.workflow where workflow_id = ?',[res.workflow_workflow_id],(error,q1) =>{
                workflows.push(q1[0].workflow_name);
            ...
            for(var i=0;i<workflows.length;i++){
                arr11[i].splice(0,0,workflows[i]);
            }
            //console.log("hhhasa")
            //console.log(paths);
            console.log(arr11)
...
app.get("/getprofile",(req,res)=>{
    ...
    con.query("select * from workflow.users where username=? ",[userid],(error,res1)=>{
        console.log(res1)
        res.send(res1)
